...
CHECKPOINT_SWITCH = False  # Whether to load from checkpoint or start fresh

START_EPISODE = 0
MAX_EPISODES = 400  # Test with 5, can increase to 100-200
MAX_STEPS = 400
...
def main():
    ...
    os.makedirs("1_Logs_RGCNN_3_WAY", exist_ok=True)
    csv_file = "1_Logs_RGCNN_3_WAY/ppo_3way_metrics_RGCNN.csv"
    ...
    if CHECKPOINT_SWITCH:
        ...
        print("Loaded pretrained critic for 3-way")
    ...
    for episode in range(START_EPISODE, START_EPISODE+MAX_EPISODES):
        ...
        direction_configs = {
            "E":  (253, VANS, 45),
            "N":  (43, CARS, 62),
            "S":  (223, CARS, 55)   # North: Car, spawn id 52, speed 48
        }
        ...
        for step in trange(MAX_STEPS, desc=f"Episode {episode+1} Steps", leave=False):
            ...
            for v in vehicles:
                ...
                obs = build_graph(vehicles, intersection_loc, ["E", "S", "N"], ego_vehicle=v)
        ...
        if (episode+1) % 1 == 0:
            os.makedirs("1_RL_Models_3WAY_RGCNN", exist_ok=True)
            torch.save(model.state_dict(), f"1_RL_Models_3WAY_RGCNN/ppo_ep{episode+1}.pth")
