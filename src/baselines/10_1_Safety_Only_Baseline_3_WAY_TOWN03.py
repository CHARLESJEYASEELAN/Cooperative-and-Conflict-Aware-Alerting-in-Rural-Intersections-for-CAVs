...
def configure_traffic_manager(tm, vehicles):
    for vehicle in vehicles:
        ...
        tm.distance_to_leading_vehicle(vehicle, 2.0) #was 2.0
        tm.vehicle_percentage_speed_difference(vehicle, +30) #was -75
...
def main():
    ...
    '''
    csv_file = f"10_1_Safety_Only_Baseline_3WAY_metrics.csv"
    ...
    # Visual zones
    draw_radius_circle(world, intersection_loc, radius=50.0,
                       color=carla.Color(0, 255, 255), z_offset=0.1)
    draw_radius_circle(world, intersection_loc, radius=20.0,
                       color=carla.Color(255, 0, 255), z_offset=0.2)
    draw_direction_lines(world, intersection_loc, ["N", "S", "E", "W"], length=30.0, z=2.0)
    ...
    for episode in range(NUM_DEMO_EPISODES):
        ...
        for step in range(MAX_STEPS):
            ...
            for v in alive_vehicles:
                ...
                if someone_inside_clear and dist <= STOP_LINE_RADIUS + 2.0: #was 3.0
                    actions_dict[v.id] = 0
                    continue
                ...
                speed = np.linalg.norm([v.get_velocity().x, v.get_velocity().y, v.get_velocity().z])
                if dist <= STOP_LINE_RADIUS + 20.0 and speed > 3.0:
                    actions_dict[v.id] = 1
                    continue
            ...
            if non_stop_vehicles:
                ...
                allowed_move_ids = {v.id for v in sorted_by_dist[:3]} #was 3
